...
- **Nothing calls `chmod`.** The executable bit existed for the hook and for nothing else; a tool
  writing markdown, YAML and JSONL never needs one. Grepping for `.git` instead would have to
  carve out `.github/prompts`, and an assertion with an exception in it gets edited rather than
  obeyed.
