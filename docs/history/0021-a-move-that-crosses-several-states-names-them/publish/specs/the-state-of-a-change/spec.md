...
**What is not refused is still said.** A move that crosses more than one edge names the states
it passed, on a second line under the transition:
...
Those states were **passed over, never performed**, and the distinction is the whole of it: the
ledger gets the one transition it was told about, and naming the rest is a rendering. A flag that
appended an event per edge was the other candidate and is refused — it would write a record of
transitions nobody made, which is the failure the ledger exists to prevent rather than a
convenience it lacks. The true audit trail of a six-state jump is one line saying a six-state
jump happened.

It is said because a one-edge move and a six-edge move otherwise differed in the output by a
single word. `molly move 0003 deployed` typed where `review` was meant read exactly like a
deliberate jump, at the one moment it was still free to correct — and afterwards the ledger holds
a line that is entirely true and reads, to anybody folding it later, as a change that was never
reviewed. The distance is arithmetic over a sequence the tool owns, so this is
[what a command may never do silently](../what-a-command-may-never-do-silently/spec.md) applied to
something the tool already knew and did not show.

A move that goes back names them in the direction it travelled, because somebody reading
`deployed → review` is walking backwards and a list in forward order describes a journey nobody
took. An adjacent move says nothing, and neither does one that stays: a clause that fired when
nothing was skipped would train people to read past it.
...
Naming what a move skipped is not that seam and must not be mistaken for it. Reporting is what a
tool with no opinion about the order can honestly do; refusing is what a slice will do when there
is one to ask. The exit code stays `0`, asserted, so the day policy arrives it has to break that
