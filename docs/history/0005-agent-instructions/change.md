...
A corpus arrives with the instructions an agent needs to work in it, in the places agents
already look, in the one format every major tool reads. `molly init` installs them alongside the
corpus; `molly agents` reinstalls them after an upgrade.
...
implementation ignores what it does not recognise. Two fields is the contract all of them
honour, so it is the whole contract.

## Two directories, and why that is the coverage rather than the limit

Each of these was read from that tool's own documentation on 2026-08-11, not from a comparison
table — two of the rows below contradict the popular one.
...
The default installs both, and between them that is every major tool rather than a sample of
them. Nine converged on a shared root; the two exceptions are a matched pair, Claude Code reading
only its own directory and Cline reading Claude Code's, so one of the two defaults catches each.
...
words on the small set of things a capable model gets wrong here **because another tool taught
it otherwise**: that a document is replaced whole and there is no delta format, that the tool
...
difference in it is somebody's own configuration rather than drift, and a check that failed on
it would be a check people learn to ignore.

Claude Code alone gets this, because its project settings file is the one whose location and
shape are documented. The spec's `allowed-tools` field would carry the grant into every tool and
...
that `--check` calls current. Recorded in the roadmap.

**Nothing verifies that the instructions were followed.** An agent that edits `docs/specs/`
directly is not refused — it shows up as a change to accumulated truth with no publication
behind it, visible in the diff a person reads. The tool approves nothing, here as everywhere.

**Skills are not translated.** They are written in English while instructing the agent to write
the corpus in its own language, which is the cheap half of the problem and the half that
matters: an English instruction produces Ukrainian documents, and a Ukrainian corpus with
English documents in it is what happens without it.
