...
**And the corpus paths a skill names are checked against a real corpus**, rather than against the
generator that wrote them. This is the gap `--check` cannot close by its nature: it regenerates
and compares, so a claim the generator is *also* making is a claim both sides agree on and neither
tests. That is how the skills went on pointing at a configuration file inside the corpus after the
file had moved out of it — nothing was out of date, and the text was wrong. The paths are read out
of the installed skills and resolved against a corpus `init` has just made; `conventions.md` is
excluded by name, because the skill says *if it is there* and its absence is the ordinary case.
