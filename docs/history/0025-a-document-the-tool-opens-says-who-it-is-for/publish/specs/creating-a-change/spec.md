...
Each opens by naming the reader it is written for, which is what decides where a sentence goes
and is the subject of its own section below — the only instruction the templates carry that is not
about the section it heads.
...
# A document says who it is for, because a subject is something a writer classifies

The four documents are one unit of intent cut four ways, and the cut used to be described by
**subject**: what and why in `change.md`, how in `plan.md`. That describes the result correctly and
is unusable while writing, because it asks an author to classify a sentence they have just written —
and *why* is absorptive. Every architectural argument can be phrased as a reason, truthfully, and it
then satisfies the stated criterion exactly. *"The two requirements pull against each other, and that
is why this change publishes a decision rather than making a local choice"* is a why. It is also a
statement about the corpus's own bookkeeping, addressed to a reader who knows what a decision
document is, and it belongs in `plan.md`.

So each template opens by naming a **reader** instead:

| document | written for | so a sentence belongs only if |
| --- | --- | --- |
| `change.md` | somebody deciding whether the work should happen, who will not open the codebase and does not know this corpus exists | the decision changes when it stops being true |
| `plan.md` | whoever will build it, with the codebase open | somebody implementing this would need it, including every reason that needed the codebase to explain |
| `tasks.md` | whoever picks the work up part-done | somebody could look at the repository and say whether it is finished |
| `tests.md` | whoever has to believe the claim afterwards | somebody could observe it and answer yes or no |

*Would this reader need it* has an answer, and the answer does not move when the sentence is
rephrased. *Is this what, or how* does not, which is how an author who has read the rule breaks it
without noticing. The failure is not what the phrase "technical detail" suggests either — protocol
names and library choices are easy to spot and get caught. What accumulates in `change.md` is
process about the change's own paperwork, sequencing arguments, testing strategy, and construction
detail wearing a link.

**Each template names the destination as well as the test**, and that half is not decoration. Told
only what may *not* go in a document, an author deletes the material rather than moving it, and the
reasoning is lost instead of relocated — worse than the leak it corrects, because a leak is visible
in the wrong document and a deletion is visible nowhere. `plan.md` says the same thing from the
other side: the reasons that would not fit `change.md` land here rather than nowhere.

The same sentence opens a capability and a roadmap slice, for the same reason and with less at
stake. Every document this tool opens for somebody to fill in says who will read it.

# What a change cannot answer is part of the change

`change.md` carries a third heading, `# What is not settled`, and a question goes under it in the
words somebody would ask it. An answer is recorded by rewriting the document it belongs in and
deleting the question — an answer that stays in the conversation it was given in is one the next
reader does not have. Most changes settle everything before review and "nothing" is the answer when
they do, the same shape as the standing constraint in `plan.md` that most changes also do not leave.

**It is a heading, and nothing else.** No command, no ledger event, no hash pinning an answer to the
documents it was given against. That machinery existed and was removed, and the reasoning is in
[instructions any agent can read](../agent-instructions/spec.md): a question is text, so a question
appearing and being answered is a diff, and git records it with author, timestamp and surroundings.
The feature most defended in it — detecting an answer that never reached the documents — solved a
problem that existed only because there was a second place to record answers. A heading is that
feature with no second place.

Nothing refuses a change for holding one, and the drafting skill says so plainly, because an agent
told *the tool will stop you* about something the tool does not stop is one that stops trusting the
rest of its instructions. The gate is a person declining to approve a change they can see is
unresolved. What the heading adds is that they can see it: an open question is on the face of the
document the approval is about, rather than in a session that has ended.

# A change is revised by rewriting it

A change that survives a conversation is written twice, and nothing said what the second writing
should look like. Appending is the locally sensible move — the correction is the new information and
recording it looks like honesty — and it produces a `change.md` with a dated *Widened* section, a
`plan.md` opening on a *Corrections* block that contradicts the text below it, and a `tasks.md` of
struck-through items with replacements beside them. Every individual edit is reasonable. The
document is a diff of somebody's thinking, and a reviewer cannot tell which half is current.

**The documents state what is in force, as though they had always said it.** How they came to say it
is in git and in the ledger, which record it better than prose can, and a document that records it
as well is a second account of the same fact — the shape [the ledger is the record and everything
else is a projection](../../decisions/the-ledger-is-the-record-and-everything-else-is-a-projection.md)
rules out everywhere else in this corpus. The alternative that lost belongs in `plan.md` with the
argument that settled it; *when* it lost does not.

This is stated in the templates rather than only in a skill because it is self-reinforcing: once a
document holds one correction block, the next revision appends to that block, since the block is now
the place corrections go. The template is the text in front of the author at the keystroke where it
happens, and `tasks.md` carries the instance that is hardest to argue with — a task no longer wanted
is deleted rather than struck through.
...
**`alters:` is resolved, late and without failing.** It names documents in the knowledge base, and
until something read that back there was nothing to check against — a check that passes for want of
anything to fail it is indistinguishable from one that does not work. Now `molly status` reports an
in-flight change naming a document that is not filed.

It does not fail, and the reason is the field's meaning: `alters:` is what the author intends to
replace, and the document may be arriving in this very change or in one still in flight. The tool
cannot tell a typo from a plan, so it says what it sees on every run and leaves the judgement where
it belongs. An **archived** change is not asked — a published change naming a document since
removed is the finished shape of the link, and reporting it would turn every correct publication
into a finding.

It is still not checked at creation, and deliberately: refusing there would refuse an author for
naming a document they are about to write.
...
**Placement is not format, and the line between them is where the engine's reading would begin.** A
template naming its reader, the destination of a sentence that fails it, or the route a correction
takes is saying where material goes — and the four documents and what each is for are this tool's
model, so where material goes is this tool's to state. A keyword, a required section, a heading that
presumes an acceptance-criteria methodology, or any rule whose observance the engine would have to
read a body to check is form, and stays refused.

That last clause is the test, and it is checkable rather than argued: nothing added to a template
may give `molly status` or `molly publish` a reason to look below the frontmatter. Neither does. A
change whose *What is not settled* holds five questions creates, moves and publishes exactly like
one that holds none — which is also what keeps `# What is not settled` a place to write rather than
a field to satisfy.
