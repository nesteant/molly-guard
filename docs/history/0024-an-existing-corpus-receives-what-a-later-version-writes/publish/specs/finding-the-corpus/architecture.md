...
**`init` branches once, on whether `corpusAt` found something, and the branch is small.** The
configuration is written only where there is not one; the corpus directory, and the language in
the summary, come from the corpus that was found rather than from the flags. Everything after
that is the loop that was already there — `mkdir -p`, then `place`, which writes only where
nothing is and collects what it found. **The behaviour a completing run wants is what the loop
already did**; what changed is being allowed to reach it.

Two refusals survive and both are about the configuration: `--root` naming a directory other than
the corpus that is here, and `--lang`, which would be a request to rewrite a file this run leaves
alone. The flag reaches `init` **as given** rather than defaulted, because only the caller's
silence distinguishes *complete the corpus that is here* from *make a second one*.

**`skeleton.ts` holds what `init` writes**, and holds it because a second reader arrived: `status`
reports what is absent. Two lists of the same thing drift the day one of them gains a file. The
areas are not in it — they come from the table in core, where an area is declared — so it is the
rest: the explainers belonging to no area, the ledger, and the attributes file. `conventions.md`
is deliberately absent from it, because a corpus without one is a project with no rules of its
own and reporting it would turn an invitation into a nag.

**The finding renders above the empty-listing branch, not inside it.** That branch enumerates the
finding kinds allowed to contradict *no changes yet*, so a kind it does not enumerate is a kind it
hides — the failure `what a command may never do silently` already records about this exact
branch, one kind further along. A corpus-level finding does not contradict the sentence; it sits
above it and is true either way.
...
**A corpus completed rather than refused** — `.gitattributes` removed and regained, an area
removed and regained, the ledger and the configuration hashed before and after, and a hand-written
explainer surviving byte for byte. The two runs are asserted to print different things, and a run
with nothing to add is asserted to say so. `status` names the gap, does not fail for it, carries it
in `--json`, and stops naming it once the run has closed it.
