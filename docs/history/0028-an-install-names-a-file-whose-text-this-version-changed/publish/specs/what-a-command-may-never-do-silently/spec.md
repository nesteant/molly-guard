...
# A command that moved something says what pointed at it

Publishing archives a change, and until this the corpus was not told. Every relative link that
resolved into `changes/<name>/` stopped resolving the moment it became `history/<name>/`, and the
command that moved it said nothing — so the break surfaced days later as a red build in somebody
else's change, and the file that had to be edited had nothing to do with the change that was
published.

Three publications in one adopting repository produced three variants of it. The one that cannot
be worked around is a link *from* a live change *into* the one being published: the source is a
change still being drafted and it is right to check it, the link was correct when it was written,
and publishing invalidated it. **The author had no path they could have chosen that would have
survived**, which is what makes this the tool's to report rather than a convention to remember.

**Naming is not repairing, and the difference is who edits the file.** This tool does not rewrite
a reference to keep itself correct — [publishing a change](../publishing-a-change/spec.md) sets
out why, and it is the same argument as everywhere else here: an edit nobody asked for, made to
a document under review, is the write this rule forbids even when the edit would have been right.

It is also not a refusal. The write has already happened by the time a reference can be checked
against its new state, and a publication that reported a consequence is not a publication that
failed.
...
- **A file whose text this version has changed is named too, and still kept.** Keeping it was
  never in question; the report was. *It already had everything this version writes* is a claim
  about contents drawn from a check on their absence, and where the two disagree the sentence is
  simply false — which is what happened on the release whose whole delivery was a paragraph added
  to one explainer, run against a corpus holding the previous version's copy of it. What existence
  establishes is that every file this version writes was already there, and that is now all the
  line claims. The rest is a named path and the operator's decision.

**Three of the files placed are not compared, and the exclusions are the load-bearing part.** A
finding on the healthy case is a finding people learn to read past, which is the shape this corpus
has refused before. `conventions.md` is placed empty *as an invitation*, so a project that filled
it in is the outcome the tool asked for. `.gitattributes` already has its own named remedy — the
missing merge line, printed with the line to add — and one file wants one message. The ledger is
placed empty and holds data rather than prose, so comparing it could only ever say *differs*, which
carries no information; that is the general rule, stated where the write happens: a file the tool
places empty has no version to be behind.

**Reported, never replaced**, and the asymmetry with the agent instructions is ownership rather
than care. `molly agents` rewrites what differs because those are this tool's own
`molly`-namespaced files. `init` writes into a directory the tool did not make, where a file is the
project's from the moment it exists — and this corpus was asked for the replacement and refused it
under `nothing an install finds is overwritten, lost or hidden`. The remedy offered is the one
already offered for a kept explainer: it is prose nothing reads, so delete it and run again.
