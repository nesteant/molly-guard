...
**A file the tool did not write is not overwritten, in any command.** `.claude/settings.json` is
merged, `init` skips, `publish` writes only into a corpus that is its own. The next command that
writes outside a corpus answers this question before it is written, rather than after somebody
loses a file.
