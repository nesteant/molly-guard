...
a half-made document behind when the person walks away.

**Abandoning is a refusal, not a success.** Ctrl+C leaves nothing changed and exits `1`. It is
tempting to call it `0` because giving up is not a *defect* — but `0` is the code that says the
command did what it was asked, and nothing was moved, written or filed. The three codes already
carry this distinction: `2` is the defect, `1` is the refusal, and an abandoned prompt is the
second.
